...
import { Invokes, LibraryLayoutMode } from '../components/ui/AppProperties';
...
export function useFileOperations(
  ...
  const executeDelete = useCallback(
    async (pathsToDelete: Array<string>, options = { includeAssociated: false }) => {
      ...
      const { libraryLayoutMode, libraryPreviewActivePath, setUI } = useUIStore.getState();

      const explicitDeletedPaths = new Set(pathsToDelete);
      const deletedPhysicalPaths = new Set(pathsToDelete.filter((path) => !path.includes('?vc=')));
      const isDeletedPath = (path: string) =>
        explicitDeletedPaths.has(path) || deletedPhysicalPaths.has(path.split('?vc=')[0]);
      const remainingPreviewSelection =
        !selectedImage && libraryLayoutMode === LibraryLayoutMode.Preview
          ? multiSelectedPaths.filter((path) => !isDeletedPath(path))
          : null;
      const preservedPreviewSelection =
        remainingPreviewSelection &&
        remainingPreviewSelection.length > 0 &&
        remainingPreviewSelection.length < multiSelectedPaths.length
          ? remainingPreviewSelection
          : null;
      ...
      if (activePath) {
        if (isDeletedPath(activePath)) {
          const currentIndex = sortedImageList.findIndex((img) => img.path === activePath);
          if (currentIndex !== -1) {
            const nextCandidate = sortedImageList.slice(currentIndex + 1).find((img) => !isDeletedPath(img.path));
            ...
            if (nextCandidate) {
              ...
              const prevCandidate = sortedImageList
                ...
                .reverse()
                .find((img) => !isDeletedPath(img.path));
      ...
      try {
        ...
        loupeStore.loupes.forEach((loupe) => {
          if (isDeletedPath(loupe.path)) loupeStore.closeLoupe(loupe.id);
        });

        await refreshImageList();

        const existingPaths = new Set(useLibraryStore.getState().imageList.map((image) => image.path));
        if (selectedImage) {
          if (isDeletedPath(selectedImage.path)) {
            if (nextImagePath && existingPaths.has(nextImagePath)) {
              handleImageSelect(nextImagePath);
            } else {
              handleBackToLibrary();
            }
          }
        } else if (preservedPreviewSelection) {
          const survivingSelection = preservedPreviewSelection.filter((path) => existingPaths.has(path));

          if (survivingSelection.length > 0) {
            const nextActivePath =
              libraryActivePath && survivingSelection.includes(libraryActivePath)
                ? libraryActivePath
                : survivingSelection[0];
            setLibrary({
              multiSelectedPaths: survivingSelection,
              libraryActivePath: nextActivePath,
              selectionAnchorPath: nextActivePath,
            });
            if (libraryPreviewActivePath && !survivingSelection.includes(libraryPreviewActivePath)) {
              setUI({ libraryPreviewActivePath: null });
            }
          } else if (nextImagePath && existingPaths.has(nextImagePath)) {
  ...
  const handleDeleteSelected = useCallback(
    (paths?: string[]) => {
      ...
      const pathsToDelete = paths || multiSelectedPaths;
